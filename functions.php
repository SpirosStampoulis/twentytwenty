...
function alienspecies_create_post_type()
{
    register_post_type('Alien Species',
        array(
            'labels' => array(
                'name' => __( 'Alien Fish' ),
                'featured_image' => __( 'Logo (329px x 100px)' ),
                ...
                'singular_name' => __( 'Alien Fish Review' )),
            'public' => true,
            'has_archive' => false,
            'rewrite' => array('slug' => 'anbieter','with_front' => false),
            'supports' => array('title','thumbnail','comments')
        )
    );
}
add_action('init', 'alienspecies_create_post_type');

/**
 * Alien Dropdown maps
 */
function alienmaps_shortcode( $atts, $content = null  )
{
    ob_start();
    ...
    $value_no_whitespaces = preg_replace( '/\s*;\s*/', ';', filter_var( $attributes['value'] ) );
    $value_array = explode( ';', $value_no_whitespaces );

    $title_no_whitespaces = preg_replace( '/\s*;\s*/', ';', filter_var( $attributes['dropdown_title'] ) );
    $title_array = explode( ';', $title_no_whitespaces );

    $output .= '<div class="dropdown-wrapper">';
    $output .= '<div class="dropdown">';
    $output .= '<select id="tablemapselector">';

    $value_length = count($value_array);
    for ($i = 0; $i < $value_length; $i++) {
        print $value_length[$i];
        $output .= '<option value="'.$value_array[$i].'">'.$title_array[$i].'</option>';
    }
...
add_shortcode( 'alienmaps', 'alienmaps_shortcode' );